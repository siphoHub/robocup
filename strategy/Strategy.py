...
class Strategy():
    def __init__(self, world):
        ...
        self.log_file_path = Path("logs") / "game_states.log"
        try:
...
## Helpers ##
    ...
    def GameStates(self,world):
        ...
        log_line = f"GameState={state},BallPos={self.ball_2d},OurMinDist={self.min_teammate_ball_dist:.2f},OppMinDist={self.min_opponent_ball_dist:.2f}"
        print(log_line)
        try:
    ...
    def teammate_closest_to_opponent_goal(self):
        goal = np.array((15.0, 0.0)) if self.side == 0 else np.array((-15.0, 0.0))
        best_pos = None
        best_dist = math.inf

        for idx, pos in enumerate(self.teammate_positions):
            if pos is None or idx + 1 == self.player_unum:
                continue
            dist = np.linalg.norm(goal - np.array(pos))
            if dist < best_dist:
                best_dist = dist
                best_pos = pos
...
##Execut All ##   
    ...
    def Execute(self,agent,world,state=None):
        ...
        my_ball_dist = np.linalg.norm(np.array(self.my_head_pos_2d) - np.array(self.ball_2d))

        #Attack strat/ tikki Takka
        if state==1: 
            if self.player_unum == 3:
                if my_ball_dist > 0.2:
                    return agent.move(world.ball_abs_pos[:2], avoid_obstacles=False, is_aggressive=True)

                teammate_goal_target = self.teammate_closest_to_opponent_goal()
                if teammate_goal_target is not None:
                    return agent.kickTarget(self, self.mypos, teammate_goal_target)

                return agent.kickTarget(self, self.mypos, (15.0, 0.0) if self.side == 0 else (-15.0, 0.0))

            elif self.player_unum==self.active_player_unum:
                if my_ball_dist > 0.2:
                    return agent.move(world.ball_abs_pos[:2], avoid_obstacles=False, is_aggressive=True)
        ...
        #Defend
            ...
            if self.player_unum==self.active_player_unum:
                return agent.kick() if np.linalg.norm(np.array(self.ball_2d)-np.array(self.my_head_pos_2d))<0.7 else agent.move(world.ball_abs_pos[:2], avoid_obstacles=False, is_aggressive=True)
